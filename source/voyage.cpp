...
/**
 * \brief setter du vector contenant les Arrets du Voyage et les mets en ordre de numero de sequence
 * \param[in] un vecteur contenant tous les Arrets du Voyage en ordre
 */
void Voyage::setArrets(std::vector<Arret>& resultat)
{
	PRECONDITION(!(resultat.empty()));
	std::sort(resultat.begin(), resultat.end());

	Arret tmp = resultat[0];
	for (std::vector<Arret>::iterator it = std::next(resultat.begin(), 1);
			it != resultat.end(); ++it)
	{
		if (tmp.getHeureArrivee() == it->getHeureArrivee())
		{
