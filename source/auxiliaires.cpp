...
/**
 * \brief Prend un vecteur delimiter et le transforme en un vecteur de string
 * \param[in] un string
 * \param[in] un char delimiteur dans le string
 * \return le vecteur contenant le string sans les delimiteur
 */
...
/**
 * \brief lis un fichier et le transforme en un vecteur 2d
 * \param[in] un string avec le nom du fichier a lire
 * \param[in] un vecteur 2d qui contiendra les informations
 * \param[in] le delimiteur contenu dans le fichier
 * \param[in] un booleen pour savoir si la premiere ligne du fichier doit etre jete. True si elle doit etre jete et false si on veut la garder
 */
		...
		bool rm_entete)
{
	...
	try
	{
		...
		if (rm_entete)
		{
			...
			{
				resultats.push_back(split(tmp, delimiteur));
			}
		}
	...
	{
		ifs.close();
		throw std::runtime_error("Le fichier n'a pu etre ouvert");
	}
...
bool Date::validerDate(unsigned int p_an, unsigned int p_mois, unsigned int p_jour)
{
	...
	if ((p_an % 100) == 0 and p_mois == 2 and p_jour > 28 and (p_an % 400) != 0)
	{
		return false;
	}

	if ((p_an % 4) == 0 and p_mois == 2 and p_jour > 29 and (p_an % 100) != 0)
	{
		return false;
	}
